...
export class PdfLoader {
  constructor(element, param = {}) {
    ...
    this.realHeader = param.realHeader;
    this.realFooter = param.realFooter;
    this.footer = param.footer;
  ...
  getPdf() {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      for (let i = 0; i < pages.length; ++i) {
        ...
        // 1. 首先清除整个页面内容，防止历史内容残留
        this.addBlank(0, 0, this.A4_WIDTH, this.A4_HEIGHT, pdf);
        // 根据分页位置新增图片，要排除页眉和顶部留白
        this.addImage(baseX,  pdfHeaderH - pages[i], pdf, data, width, height);

        // 将 内容 与 页眉之间留空留白的部分进行遮白处理
        // this.addBlank(0, pdfHeaderH, this.A4_WIDTH  - baseY - pdfFooterH, baseY, pdf);
        // 3. 内容区域遮白处理（调整参数确保完全覆盖）
        // 将 内容 与 页眉之间留空留白的部分进行遮白处理
        this.addBlank(0, 0, this.A4_WIDTH, pdfHeaderH + baseY, pdf);

        // 将 内容 与 页脚之间留空留白的部分进行遮白处理
        // this.addBlank(0, this.A4_HEIGHT - baseY - pdfFooterH, this.A4_WIDTH, baseY, pdf);
        // 将 内容 与 页脚之间留空留白的部分进行遮白处理
        const footerY = this.A4_HEIGHT - pdfFooterH - baseY;
        this.addBlank(0, footerY, this.A4_WIDTH, pdfFooterH + baseY, pdf);
        ...
        if (i < pages.length - 1) {
          ...
          // 对多余的内容部分进行遮白
          // this.addBlank(0, baseY + imageHeight + pdfHeaderH, this.A4_WIDTH, this.A4_HEIGHT - imageHeight - pdfHeaderH - pdfFooterH, pdf);
          const contentBottom = baseY + imageHeight + pdfHeaderH;
          // 修复：遮白高度精确计算到页脚区域
          this.addBlank(0, contentBottom, this.A4_WIDTH, this.A4_HEIGHT - contentBottom - pdfFooterH, pdf);
      ...
      console.log('总页数getNumberOfPages：'+pdf.getNumberOfPages())
      for (let i = 1; i < pdf.getNumberOfPages(); ++i) {//添加真正的页眉页脚
        // 添加页眉
        await this.addHeader(i + 1, this.header, pdf, this.A4_WIDTH);
  ...
  updatePos(eleHeight, top) {
    ...
    // 修复：添加最小高度阈值，过滤无效分页
    const minHeight = 10; // 小于10px的元素不触发分页
    if (eleHeight < minHeight) {
      return false;
    }
    ...
    if (top - pageH >= originalPageHeight ) {
      ...
      console.log("%c [ 分页了1 ]: ", "color: #bf2c9f; background: yellow; font-size: 13px;", "分页了");
      return true;
    }


    // 当元素底部接近分页点(90%)时提前分页
    const threshold = originalPageHeight * 0.95;
    if (top + eleHeight - pageH > threshold && top != pageH) {
      console.log("%c [ 提前分页 ]: ", "color: #bf2c9f; background: pink; font-size: 13px;", "提前分页");
      this.pages.push(top);
      return true;
    }
    ...
    // top！=pageH这个条件是防止多个元素嵌套情况下，他们top是一样的
      ...
      console.log("%c [ 分页了2 ]: ", "color: #bf2c9f; background: pink; font-size: 13px;", "分页了");
      // this.pages.push(pageH + originalPageHeight); // 正确做法：按整页高度分页
      this.pages.push(top);
      return true;
    }
    /*const elementBottom = top + eleHeight;
    if (elementBottom - pageH > originalPageHeight) {
      this.pages.push(pageH + originalPageHeight); // 正确做法：按整页高度分页
    }*/
  }
  ...
  async addFooter(pageSize, pageNo, footer, pdf, contentWidth) {
    ...
    if (pageNo === 1 && this.footerFirst) {
      // 第一页
      const { height, data } = await this.toCanvas(footer, contentWidth);
