...
export default {
  ...
  mounted() {
    if (process.env.VUE_APP_SOCKET_ENABLED !== 'true') return
    // let wsUrl = "ws://localhost:8091/ws/" + this.sysUserId;
    // let wsUrl = "ws://" + window.location.hostname + ":8091/ws/" + this.sysUserId;
    let wsUrl = `${location.protocol === 'https' ? 'wss' : 'ws'}://${location.host}/ws/` + this.sysUserId;
    this.$store.dispatch("startWebSocket", {url: wsUrl, user: this.$options.name}).then(() => {
      // 添加socket通知监听
  ...
  methods: {
    // 收到消息处理
    getSocketData (e) {
      let data = JSON.parse(JSON.parse(e.detail.data).data)
